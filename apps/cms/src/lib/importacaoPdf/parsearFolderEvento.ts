  ...
  const nome =
    idxData > 1
      ? linhas
          ...
          .replace(/\s+/g, " ")
      : null;
  const dataInicioISO = idxData >= 0 && linhas[idxData + 1] ? dataPtParaISO(linhas[idxData + 1]) : null;

  const idxCarga = linhas.findIndex((l) => chaveDeLinha(l) === "CARGAHORARIA");
  const cargaHoraria = idxCarga >= 0 ? (linhas[idxCarga + 1] ?? null) : null;
    ...
    if (m) {
      ...
      if (resto.length > 0) {
        ...
        for (let i = idxTrilha + 1; i < linhas.length && !/[.!?]$/.test(continuacao.at(-1)!); i++) {
          if (ehLinhaMarcador(linhas[i])) break;
          continuacao.push(linhas[i]);
        }
...
function parsearSobre(paginas: string[]): string[] {
  ...
  if (idx < 0) return [];
  const linhas = linhasDePagina(paginas[idx]);
...
function parsearObjetivoPublico(paginas: string[]): { objetivo: string[]; publicoAlvo: string[] } {
  ...
  function fatia(chaveInicio: string, chavesFim: string[]): string[] {
    ...
    for (let j = i + 1; j < linhas.length; j++) {
      if (ehLinhaMarcador(linhas[j]) && chavesFim.some((c) => chaveDeLinha(linhas[j]).startsWith(c))) {
        fim = j;
        break;
      }
    }
...
function parsearSessoes(paginas: string[]): SessaoConteudoFolder[] {
  ...
  for (let p = 0; p < paginas.length; p++) {
    if (!chaveDeLinha(paginas[p].split("\n").slice(0, 4).join("")).includes("OQUEVOCEAPRENDERA")) {
      continue;
    }
    const linhas = linhasDePagina(paginas[p]);
...
function parsearReplay(paginas: string[]): { replayDisponivel: boolean; prazoReplay: string | null } {
  for (const pagina of paginas) {
    ...
    if (i < 0) continue;
    const trecho: string[] = [];
    for (let j = i + 1; j < linhas.length && !ehLinhaMarcador(linhas[j]); j++) {
...
export function parsearFolderEvento(paginas: string[]): DadosFolderEvento {
  const capa = paginas.length > 0 ? parsearCapa(paginas[0]) : null;
